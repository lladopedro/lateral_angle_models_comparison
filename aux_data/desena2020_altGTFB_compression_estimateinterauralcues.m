function [interauralcues] = desena2020_altGTFB_compression_estimateinterauralcues(bin_stim,fs)
%Work in progress
...
for isig = 1:Nsignals
    ...
    gtfbout(isig).left = gtfbout(isig).left.*abs(gtfbout(isig).left).^0.4;%(1/2);
    gtfbout(isig).right = gtfbout(isig).right.*abs(gtfbout(isig).right).^0.4;%(1/2);
