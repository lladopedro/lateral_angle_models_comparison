% AUTHOR: Pedro Llado
% SCRIPT TO GENERATE THE PLOT SHOWING THE INTERAURAL CUES   
...
for idir = 1:length(target_angles)
    ...
    plot(cfreqs,itd_centroid*1000,'LineWidth',2);
    ...
    plot(cfreq,itd*1000,'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-1.05 1.05])
    ...
    plot(peripheral_out.cfs,itd*1000,'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-1.05 1.05])
    ylabel('ITD (ms)'); xlabel('Frequency channel')
    ...
    plot(peripheral_out.cfs,ild,'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-25 25])
    ylabel('ILD (dB)'); xlabel('Frequency channel')
    ...
    % the window size has been manipulated
    
    subplot(6,2,7); hold on;
    plot(cfs,mean(out.itd,2)*1000,'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-1.05 1.05])
    ylabel('ITD (ms)'); xlabel('Frequency channel')
    title("ITD - may2011")
    ax = gca;
    ...
    plot(cfs,mean(out.ild,2),'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-25 25])
    ylabel('ILD (dB)'); xlabel('Frequency channel')
    title("ILD - may2011")
    ax = gca;
    ...
    plot(fc,mean(ild,1),'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-25 25])
    ylabel('ILD (dB)'); xlabel('Frequency channel')
    title("ILD - dietz2011")
    ax = gca;
    ...
    plot(target.cfs,target.itd*1000,'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-1.05 1.05])
    ylabel('ITD (ms)'); xlabel('Frequency channel')
    ...
    semilogx(target.cfs,target.ild,'LineWidth',2);
    set(gca, 'XScale', 'log'); ylim([-25 25])
    ylabel('ILD (dB)'); xlabel('Frequency channel')
