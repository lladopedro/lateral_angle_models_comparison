function [out,ihc_out] = may2011pl(input,fs)
...
%   demo_may2011
%
%   Url: http://amtoolbox.org/amt-1.6.0/doc/models/may2011.php
...
%if stationary
%blockSec  = size(input,1)/fs; % Block size in seconds
%if time-dep
blockSec  = 50e-3; % Block size in seconds
...
for ii = 1 : GFB.nFilter
    ...
    switch lower(A.xcorrMethod)
        case 'power'
            % Time-based
            xcorrel = may2011_xcorrnorm(frameL,frameR,A.maxDelay,A.bXcorrDetrend,...
                              A.bXcorrNorm);
    end
    %PL
    %if timedep
    ...
    bM = transpose(argmax(xcorrel,1)); 
    
    % Refine lag position by applying parabolic interpolation
    [delta,currIC] = may2011_interpolateparabolic(xcorrel,bM);
